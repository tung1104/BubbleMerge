...
namespace Watermelon
{
    ...
    public class ProjectInitSettingsEditor : WatermelonEditor
    {
        ...
        private void LoadEditorsList()
        {
            ...
            for (int i = 0; i < initModulesProperty.arraySize; i++)
            {
                initModule = initModulesProperty.GetArrayElementAtIndex(i);

                if(initModule.objectReferenceValue != null)
                {
                    initModuleSerializedObject = new SerializedObject(initModule.objectReferenceValue);

                    initModulesEditors.Add(new InitModuleContainer(initModuleSerializedObject, Editor.CreateEditor(initModuleSerializedObject.targetObject)));
                }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if(initModulesProperty.arraySize > 0)
            {
                for (int i = 0; i < initModulesProperty.arraySize; i++)
                {
                    ...
                    if (initModule.objectReferenceValue != null)
                    {
                        ...
                        if (initModule.isExpanded)
                        {
                            if (initModulesEditors[i] != null && initModulesEditors[i].editor != null)
                            {
                                initModulesEditors[i].editor.OnInspectorGUI();
                            }
                            ...
                            if (initModulesEditors[i].isModuleInitEditor)
                            {
                                initModulesEditors[i].initModuleEditor.Buttons();
                            }
                            ...
                            if (GUILayout.Button("Remove", GUILayout.Width(90)))
                            {
                                if (EditorUtility.DisplayDialog("This object will be removed!", "Are you sure?", "Remove", "Cancel"))
                                {
                                    ...
                                    initModulesProperty.RemoveFromVariableArrayAt(i);
                        ...
                        if (GUI.Button(new Rect(moduleRect.x + moduleRect.width - 15, moduleRect.y, 12, 12), arrowUpContent, arrowButtonStyle))
                        {
                            if (i > 0)
                            {
                                bool expandState = initModulesProperty.GetArrayElementAtIndex(i - 1).isExpanded;

                                InitModuleContainer tempInitModuleContainer = initModulesEditors[i];
                                initModulesEditors[i] = initModulesEditors[i - 1];
                                initModulesEditors[i - 1] = tempInitModuleContainer;

                                initModulesProperty.MoveArrayElement(i, i - 1);

                                initModulesProperty.GetArrayElementAtIndex(i - 1).isExpanded = initModule.isExpanded;
                                initModulesProperty.GetArrayElementAtIndex(i).isExpanded = expandState;
                                serializedObject.ApplyModifiedProperties();
                            }
                        }
                        if (GUI.Button(new Rect(moduleRect.x + moduleRect.width - 15, moduleRect.y + 12, 12, 12), arrowDownContent, arrowButtonStyle))
                        {
                            if (i + 1 < initModulesProperty.arraySize)
                            {
                                bool expandState = initModulesProperty.GetArrayElementAtIndex(i + 1).isExpanded;
