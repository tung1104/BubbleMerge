...
namespace Watermelon
{
    ...
    public static class EditorStylesExtended
    {
        ...
        public static Texture2D GetTexture(string name)
        {
            ...
            if (editorSkin != null)
            {
                if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(editorSkin)))
                {
                    ...
                    string[] guids = AssetDatabase.FindAssets(name, new[] { folderPath });
                    if(guids.Length > 0)
                    {
        ...
        {
            Texture2D iconTexture = GetTexture(name);
            if(iconTexture != null)
            {
                Texture2D tempTexture = new Texture2D(iconTexture.width, iconTexture.height);
                tempTexture.SetPixels(iconTexture.GetPixels());
